...
export const getContactHistory = query({
  ...
  handler: async (ctx, args) => {
    ...
    return await ctx.db
      ...
      .withIndex("contactId_dateTime", (q) =>
        q.eq("contactId", args.contactId as Id<"contacts">),
      )
      .order("desc")
      .collect();
  },
...
export const getContactHistoryStats = query({
  ...
  handler: async (ctx, args) => {
    ...
    const allHistory = await ctx.db
      .query("contactHistory")
      .withIndex("contactId", (q) =>
        q.eq("contactId", args.contactId as Id<"contacts">),
      )
      .collect();

    const totalInteractions = allHistory.length;
    const totalDuration = allHistory.reduce(
      (sum, h) => sum + (h.duration || 0),
      0,
    );
    const lastInteraction =
      allHistory.length > 0
        ? Math.max(...allHistory.map((h) => h.dateTime))
        : null;

    const interactionTypes = allHistory.reduce(
      (acc, h) => {
